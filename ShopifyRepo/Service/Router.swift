...
protocol Router {
    ...
    func load<T: Codable>(_ session: BleedingFastURLSession, dateDecodingStrategy: JSONDecoder.DateDecodingStrategy?, expectedResultType: T.Type, completion: @escaping (_ json: T?, _ error: Error?) -> Void) -> URLSessionDataTaskProtocol?
    func load<T: Codable>(_ session: BleedingFastURLSession, decoder: JSONDecoder, expectedResultType: T.Type, completion: @escaping (_ json: T?, _ error: Error?) -> Void) -> URLSessionDataTaskProtocol?
    
    func request() -> URLRequest?
    func request(_ urlComponents: URLComponents, parameters: [String: Any]) -> URLRequest?
}

private let BleedingFastErrorKey = "RequestKitErrorKey"
...
extension Router {
    ...
    public func request() -> URLRequest? {
        ...
        if let customHeaders = configuration.customHeaders {
            customHeaders.forEach { httpHeader in
                urlRequest?.addValue(httpHeader.value, forHTTPHeaderField: httpHeader.key)
            }
        }
    ...
    public func load<T: Codable>(_ session: BleedingFastURLSession = URLSession.shared, dateDecodingStrategy: JSONDecoder.DateDecodingStrategy?, expectedResultType: T.Type, completion: @escaping (_ json: T?, _ error: Error?) -> Void) -> URLSessionDataTaskProtocol? {
    ...
    public func load<T: Codable>(_ session: BleedingFastURLSession = URLSession.shared, decoder: JSONDecoder = JSONDecoder(), expectedResultType: T.Type, completion: @escaping (_ json: T?, _ error: Error?) -> Void) -> URLSessionDataTaskProtocol? {
            ...
            if let response = response as? HTTPURLResponse {
                if response.wasSuccessful == false {
                    ...
                    if let data = data, let json = try? JSONSerialization.jsonObject(with: data, options: .mutableContainers) as? [String: Any] {
                        userInfo[BleedingFastErrorKey] = json as Any?
                    }
    ...
    public func load(_ session: BleedingFastURLSession = URLSession.shared, completion: @escaping (_ error: Error?) -> Void) -> URLSessionDataTaskProtocol? {
        guard let request = request() else {
            return nil
        }
